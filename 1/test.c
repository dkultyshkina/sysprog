...
static void *stress_send_f(void *arg) {
  ...
  while (ctx->next_data < ctx->last_data) {
    unit_assert(coro_bus_send(ctx->bus, ctx->channel, ctx->next_data++) == 0);
  }
  return NULL;
}
...
static void *stress_recv_f(void *arg) {
  ...
  while (ctx->size < ctx->capacity) {
    unit_assert(
        coro_bus_recv(ctx->bus, ctx->channel, &ctx->datas[ctx->size++]) == 0);
  }
  return NULL;
}
...
static void broadcast_start(struct ctx_broadcast *ctx, struct coro_bus *bus,
                            unsigned data) {
  ctx->bus = bus;
  ctx->data = data;
  ctx->rc = -1;
...
static void *send_v_f(void *arg) {
  ...
  ctx->is_started = true;
  ctx->rc = coro_bus_send_v(ctx->bus, ctx->channel, ctx->data, ctx->count);
  ctx->err = coro_bus_errno();
  ctx->is_done = true;
  return NULL;
}

static void send_v_start(struct ctx_send_v *ctx, struct coro_bus *bus,
                         int channel, const unsigned *data, unsigned count) {
  ctx->bus = bus;
  ctx->channel = channel;
  ctx->data = data;
  ctx->count = count;
...
static void *send_v_all_f(void *arg) {
  ...
  ctx->is_started = true;
  while (ctx->count > 0) {
    int rc = coro_bus_send_v(ctx->bus, ctx->channel, ctx->data, ctx->count);
    if (rc < 0) {
      ctx->err = coro_bus_errno();
      ctx->rc = -1;
      break;
...
static void send_v_all_start(struct ctx_send_v *ctx, struct coro_bus *bus,
                             int channel, const unsigned *data,
                             unsigned count) {
  ctx->bus = bus;
  ctx->channel = channel;
  ctx->data = data;
...
static void *recv_v_f(void *arg) {
  ...
  ctx->is_started = true;
  ctx->rc = coro_bus_recv_v(ctx->bus, ctx->channel, ctx->data, ctx->count);
  ctx->err = coro_bus_errno();
  ctx->is_done = true;
  return NULL;
}

static void recv_v_start(struct ctx_recv_v *ctx, struct coro_bus *bus,
                         int channel, unsigned *data, unsigned count) {
  ctx->bus = bus;
  ctx->channel = channel;
  ctx->data = data;
  ctx->count = count;
...
static void *recv_v_all_f(void *arg) {
  ...
  ctx->is_started = true;
  while (ctx->count > 0) {
    int rc = coro_bus_recv_v(ctx->bus, ctx->channel, ctx->data, ctx->count);
    if (rc < 0) {
      ctx->err = coro_bus_errno();
      ctx->rc = -1;
      break;
...
static void recv_v_all_start(struct ctx_recv_v *ctx, struct coro_bus *bus,
                             int channel, unsigned *data, unsigned count) {
  ctx->bus = bus;
  ctx->channel = channel;
  ctx->data = data;
